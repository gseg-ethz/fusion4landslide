...
def ransac_registration(src_pts, tgt_pts, corres, max_dist=0.5, ransac_n=4,
...
def visualize_patch_match(src_pts, tgt_pts, src_patch_pts, tgt_patch_pts, offset=[5, 0, 0],
                          src_color=[0.921, 0.569, 0], tgt_color=[0, 0.839, 1.], radius=0.5,
                          window_name="Open3D"):
    """
    ...
    if isinstance(tgt_patch_pts, list):
        ...
        for i, patch in enumerate(tgt_patch_pts):
            ...
            if i == 0:
                tgt_patch_pcd_combine = corr_pts2spheres(tgt_patch_pts_temp, radius=radius, colors=None)
            else:
                tgt_patch_pcd_temp = corr_pts2spheres(tgt_patch_pts_temp, radius=radius, colors=np.asarray(filtered_colors[i]))
                tgt_patch_pcd_combine += tgt_patch_pcd_temp
    else:
        tgt_patch_pcd_temp = copy.deepcopy(tgt_patch_pts)
        # tgt_patch_pcd_temp += offset
        tgt_patch_pcd_temp = corr_pts2spheres(tgt_patch_pcd_temp, radius=radius)
    ...
    if isinstance(src_patch_pts, list):
        ...
        for i, patch in enumerate(src_patch_pts):
            ...
            if i == 0:
                src_patch_pcd_combine = corr_pts2spheres(src_patch_pts_temp, radius=radius, colors=None)
            else:
                src_patch_pts_temp = corr_pts2spheres(src_patch_pts_temp, radius=radius, colors=np.asarray(filtered_colors[i]))
                src_patch_pcd_combine += src_patch_pts_temp
    else:
        src_patch_pts_temp = copy.deepcopy(src_patch_pts)
        src_patch_pts_temp += offset
        src_patch_pcd_temp = corr_pts2spheres(src_patch_pts_temp, radius=radius)
    ...
    # radius2 = 0.3 for brienz_tls
    src_pcd_temp.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=radius, max_nn=30))
    src_pcd_temp.paint_uniform_color(src_color)
    tgt_pcd_temp.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=radius, max_nn=30))
    ...
    # o3d.visualization.draw_geometries([src_pcd_temp, tgt_pcd_temp],
    #                                   window_name=window_name,
    #                                   # height=500, width=1000, left=10, top=10,
    #                                   # # zoom=0.69999999999999996,
    #                                   # zoom=0.9,
