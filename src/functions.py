...
# functions that are only used in the src code, but currently part of them may repeat with functions in './scripts/'
# source: [f2s3]
...
def point_cloud_tiling(config):
    """
    Preprocess the data, using one type of tiling methods or skip tiling
    options: xy_tiling, certain x, y + 10 m; hv_tiling, certain h, v + 10 m / range
    :return:
    """
    src_pts_path = osp.join(config.data_dir, 'raw_pcd', config.src_name)
    tgt_pts_path = osp.join(config.data_dir, 'raw_pcd', config.tgt_name)
    if config.tiling_type == 'xy_tiling':
        max_pts_per_tile = config.max_pts_per_tile
        min_pts_per_tile = config.min_pts_per_tile

        pcd_tiling.tile_point_clouds(src_pts_path,
                                     tgt_pts_path,
                                     max_pts_per_tile,
                                     min_pts_per_tile,
                                     bool(config.voxel_size),
                                     config.voxel_size,
                                     0.0,
                                     -1,
                                     config.tile_dir,
    ...
    elif config.tiling_type == 'hv_tiling':
        return None
    elif config.tiling_type == 'python_based_tiling':
        return None
    else:
