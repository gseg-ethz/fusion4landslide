...
import numpy as np
from src.functions import kabsch_transformation_estimation
import torch
...
class FilteringNetwork(nn.Module): 
    ...
    def filter_input(self, data, data_raw, config):
        ...
        if 'Rockfall_Simulator' in config.data_dir:
            coeff = 2.5
        else:
            coeff = 1
        inliers = torch.where(residuals < coeff * torch.median(residuals))[1]
