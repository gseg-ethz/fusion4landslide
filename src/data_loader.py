...
# https://github.com/fabiopoiesi/dip/blob/6b72d5c370793d91efd3560c9f1819f731b34da4/preprocess_3dmatch_lrf_test.py#L23
# pts_to_sample = 5000
# patch_size = 256
# voxel_size = .01
# lrf_kernel = .3 * np.sqrt(3)
# data loading, parallel
class Preprocess_Dataset(data.Dataset):
    ...
    def extract_patch(self, pt):
        ...
        if ptall.shape[1] > 10:
            ...
            if np.abs(np.linalg.norm(np.dot(v, (alpha * beta)[:, np.newaxis]))) < _EPS:
                xp = 1 / (np.linalg.norm(np.dot(v, (alpha * beta)[:, np.newaxis])) + _EPS) * np.dot(v, (alpha * beta)[:,
                                                                                                       np.newaxis])
            else:
                xp = 1 / np.linalg.norm(np.dot(v, (alpha * beta)[:, np.newaxis])) * np.dot(v, (alpha * beta)[:,
                                                                                              np.newaxis])
